...
void main() {
  ...
  test('Carga Temeraria resolves its immediate attack and empowered counter',
      () async {
    final cues = <BattleCombatAnimationCue>[];
    final controller = _controller(
      player: _battler(
        ...
        health: 30,
        attack: 1,
        abilities: const <BattlerAbility>[cargaTemerariaAbility],
      ),
      enemy: _battler(
        name: 'ENEMY',
        health: 30,
    ...
    await controller.togglePlayerAbility(
      controller.player.abilityById(BattlerAbilityId.cargaTemeraria)!,
    );

    expect(controller.enemy.health, 24);
    expect(controller.player.health, 24);
    expect(
      controller.player
          .abilityById(BattlerAbilityId.cargaTemeraria)!
  ...
  test('Marca de Caza immediate attack triggers Guante de Reto animation',
      () async {
    final cues = <BattleCombatAnimationCue>[];
    final controller = _controller(
      player: _battler(
        ...
        attack: 1,
        equippedItems: const <Item>[guanteRetoItem],
        abilities: const <BattlerAbility>[marcaDeCazaAbility],
      ),
      enemy: _battler(
        name: 'ENEMY',
        health: 30,
    ...
    await controller.togglePlayerAbility(
      controller.player.abilityById(BattlerAbilityId.marcaDeCaza)!,
    );

    expect(controller.enemy.health, 23);
    expect(controller.player.health, 28);
    expect(controller.player.hasStatus(DesafioStatus.statusId), isFalse);
